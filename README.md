# VORA-La-mobilit-urbaine-intelligente-et-contextualis-e-pour-le-Cameroun # Le transport connecté au rythme de nos réalités.
# Reformulation du besoin
Dans les métropoles camerounaises comme Douala et Yaoundé, les solutions classiques de transport à la demande échouent souvent parce qu'elles reposent sur un modèle occidental inadapté : adresses postales normées, courses exclusivement privées et forte dépendance à une connexion internet permanente. 
Le projet VORA répondra à quatre fractures concrètes du quotidien :  La fracture géographique : Remplacer le système d'adresses standard par une cartographie basée sur les points de repère populaires (carrefours, marchés, édifices connus) et les indications contextuelles (notes ou consignes vocales). 
La fracture économique et d'usage : Proposer un système hybride qui gère à la fois la course privée (dépôt) et le taxi partagé (ramassage) le long des grands axes, permettant aux usagers de réduire leurs coûts et aux chauffeurs d'optimiser leurs sièges vides.
La fracture transactionnelle : Supprimer le stress récurrent du rendu de monnaie et les tarifs à la tête du client grâce à une tarification transparente calculée en amont et un paiement direct via Mobile Money (MTN MoMo, Orange Money).  La fracture sécuritaire et technique : Garantir des trajets sûrs via des chauffeurs et véhicules identifiés, le partage de trajet par lien externe (SMS/WhatsApp), et une application ultra-légère conçue pour rester opérationnelle sur smartphones modestes et réseaux mobiles instables.
En résumé, VORA ne sera pas un clone de VTC(Véhicule de Tourisme avec Chauffeur), mais une infrastructure numérique de transport adaptée au terrain, transformant les habitudes informelles de mobilité en un service fiable, abordable et sécurisé.

# Projet tel que presente

🚖 1. LE DÉFI : VORA

Cette année, le projet de référence du Hackathon est VORA, une application de mobilité intelligente pensée pour répondre aux réalités du contexte camerounais.

L’objectif n’est pas de reproduire une simple application de VTC existante, mais de concevoir une solution innovante, utile et adaptée aux réalités locales, capable d’améliorer les déplacements urbains.

Votre solution devra notamment prendre en compte des problématiques telles que :
• la sécurité ;
• l’accessibilité ;
• le coût des transports ;
• la qualité du service ;
• l’expérience utilisateur ;
• les réalités propres aux déplacements urbains au Cameroun.

💻 2. CE QUE VOUS DEVREZ RÉALISER

Durant les 48 heures de compétition, chaque équipe devra concevoir et développer une solution mobile fonctionnelle intégrant au minimum :
• 🔐 Authentification des utilisateurs ;
• 📍 Géolocalisation ;
• 🗺️ Cartographie interactive ;
• 🚖 Réservation d'une course ;
• 🧭 Calcul automatique des itinéraires ;
• 💰 Estimation du coût d'une course ;
• 🚗 Suivi en temps réel du chauffeur ;
• 📱 Interface moderne, intuitive et responsive.

Au-delà du développement, chaque équipe devra être capable de justifier ses choix techniques et de démontrer clairement la valeur ajoutée de sa solution.

💡 3. L’INNOVATION : LE FACTEUR CLÉ

L’innovation constituera un critère majeur d’évaluation.
Votre application devra se démarquer par sa capacité à résoudre un problème réel avec une approche originale, pertinente et réalisable.

Voici quelques pistes d’innovation :

• 🤖 Utilisation de l’intelligence artificielle pour assister les utilisateurs ;
• 🚦 Prédiction des embouteillages et proposition d’itinéraires optimisés ;
• ♿ Assistance aux personnes à mobilité réduite ;
• 🔊 Annonces vocales intelligentes ;
• 🇨🇲 Adaptation aux réalités camerounaises : quartiers, repères, habitudes de déplacement, état des routes, etc. ;
• 👥 Optimisation du covoiturage et du partage de trajets ;
• 🛡️ Fonctionnalités renforçant la sécurité des passagers et des chauffeurs ;
• ⚡ Expérience utilisateur rapide, fluide et accessible.
Ne cherchez donc pas uniquement à faire fonctionner l’application. Cherchez à faire mieux que l’existant.


🤖 4. UTILISATION DE L’INTELLIGENCE ARTIFICIELLE

L’intelligence artificielle peut être utilisée comme outil d’accompagnement pour améliorer votre productivité, accélérer certaines tâches ou enrichir votre solution.

Cependant, certaines règles doivent être respectées :

• L’IA ne doit pas remplacer la réflexion, la conception et les choix techniques de votre équipe ;
• Les fonctionnalités développées avec l’aide d’une IA doivent être comprises et maîtrisées par les membres de l’équipe ;
• Le code et les contenus générés par une IA doivent être analysés, adaptés et intégrés de manière cohérente ;
• Utiliser une IA pour générer une solution complète sans réelle contribution technique ou créative de l’équipe ne constituera pas un avantage lors de l’évaluation ;
• Chaque équipe devra être capable d’expliquer son architecture, ses choix techniques et le fonctionnement de sa solution.
👉 L’IA est un outil. Votre créativité, votre réflexion et votre capacité à résoudre le problème restent au cœur du Hackathon.

🎨 5. POUR LES DESIGNERS UI/UX ET GRAPHISTES

Le rôle des designers sera essentiel dans la réussite des projets.
Vous serez notamment chargés de :
• 🎨 Concevoir une identité visuelle cohérente pour VORA ;
• 🖌️ Réaliser les maquettes UI/UX sur Figma ou tout autre outil adapté ;
• 🔄 Définir les parcours utilisateurs (User Flow) ;
• 📱 Concevoir les différentes interfaces de l’application mobile ;
• 🧩 Produire les icônes, illustrations et éléments graphiques nécessaires ;
• 🤝 Collaborer étroitement avec les développeurs pour garantir une intégration fidèle des maquettes.
Le design fera également partie de l’évaluation, notamment à travers :
• l’ergonomie  
• l’accessibilité 
• la cohérence visuelle
• la qualité de l’expérience utilisateur

💬 6. COMMUNICATION ET COLLABORATION

Le Hackathon se déroulant entièrement en ligne, chaque équipe est libre d’utiliser les outils de communication et de collaboration qui lui conviennent le mieux.

Vous pouvez notamment utiliser :
• Discord ;
• WhatsApp ;
• Google Meet ;
• GitHub ;
• Figma ;
• ou tout autre outil permettant une bonne organisation du travail.
